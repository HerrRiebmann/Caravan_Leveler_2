// Custom MPU6050 implementation that accepts any compatible device ID
// (MPU6050, MPU6500, MPU9250, clones, etc.)


/*!
 *  @file Custom_MPU6050.cpp
 *
 ...
 */
Custom_MPU6050::Custom_MPU6050(void) {}
...
bool Custom_MPU6050::begin(uint8_t i2c_address, TwoWire *wire,
                             int32_t sensor_id) {
  if (i2c_dev) {
  ...
  uint8_t deviceId = chip_id.read();  
  //Allow more vendor device ID´s
  if (deviceId == 0x00 || deviceId == 0xFF)
    // No valid response - sensor not connected properly
    return false;
 ...
 */
bool Custom_MPU6050::_init(int32_t sensor_id) {
  ...
  temp_sensor = new Custom_MPU6050_Temp(this);
  accel_sensor = new Custom_MPU6050_Accelerometer(this);
  gyro_sensor = new Custom_MPU6050_Gyro(this);
...
*/
/**************************************************************************/
void Custom_MPU6050::reset(void) {
  Adafruit_BusIO_Register power_mgmt_1 =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_1, 1);
...
*/
/**************************************************************************/
uint8_t Custom_MPU6050::getSampleRateDivisor(void) {
...
*/
/**************************************************************************/
void Custom_MPU6050::setSampleRateDivisor(uint8_t divisor) {
  Adafruit_BusIO_Register sample_rate_div =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_SMPLRT_DIV, 1);
...
*/
/**************************************************************************/
mpu6050_accel_range_t Custom_MPU6050::getAccelerometerRange(void) {
...
*/
/**************************************************************************/
void Custom_MPU6050::setAccelerometerRange(mpu6050_accel_range_t new_range) {
  Adafruit_BusIO_Register accel_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_ACCEL_CONFIG, 1);
...
*/
/**************************************************************************/
mpu6050_gyro_range_t Custom_MPU6050::getGyroRange(void) {
...
*/
/**************************************************************************/
void Custom_MPU6050::setGyroRange(mpu6050_gyro_range_t new_range) {
  Adafruit_BusIO_Register gyro_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_GYRO_CONFIG, 1);
  Adafruit_BusIO_RegisterBits gyro_range =
      Adafruit_BusIO_RegisterBits(&gyro_config, 2, 3);
...
*/
/**************************************************************************/
void Custom_MPU6050::setClock(mpu6050_clock_select_t new_clock) {
...
*/
/**************************************************************************/
mpu6050_clock_select_t Custom_MPU6050::getClock(void) {
  Adafruit_BusIO_Register pwr_mgmt =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_1, 1);
 ...
 */
/**************************************************************************/
mpu6050_fsync_out_t Custom_MPU6050::getFsyncSampleOutput(void) {
...
*/
/**************************************************************************/
void Custom_MPU6050::setFsyncSampleOutput(mpu6050_fsync_out_t fsync_output) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_CONFIG, 1);
  Adafruit_BusIO_RegisterBits fsync_out =
      Adafruit_BusIO_RegisterBits(&config, 3, 3);
 ...
 */
/**************************************************************************/
mpu6050_bandwidth_t Custom_MPU6050::getFilterBandwidth(void) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_CONFIG, 1);
 ...
 */
/**************************************************************************/
void Custom_MPU6050::setFilterBandwidth(mpu6050_bandwidth_t bandwidth) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_CONFIG, 1);
 ...
 */
/**************************************************************************/
mpu6050_highpass_t Custom_MPU6050::getHighPassFilter(void) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_ACCEL_CONFIG, 1);
 ...
 */
/**************************************************************************/
void Custom_MPU6050::setHighPassFilter(mpu6050_highpass_t bandwidth) {
  Adafruit_BusIO_Register config =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_ACCEL_CONFIG, 1);
...
*/
/**************************************************************************/
void Custom_MPU6050::setInterruptPinPolarity(bool active_low) {
...
*/
/**************************************************************************/
void Custom_MPU6050::setInterruptPinLatch(bool held) {
  Adafruit_BusIO_Register int_pin_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_INT_PIN_CONFIG, 1);
...
*/
/**************************************************************************/
void Custom_MPU6050::setMotionInterrupt(bool active) {
 ...
 */
/**************************************************************************/
bool Custom_MPU6050::getMotionInterruptStatus(void) {
 ...
 */
/**************************************************************************/
void Custom_MPU6050::setMotionDetectionThreshold(uint8_t thr) {
 ...
 */
/**************************************************************************/
void Custom_MPU6050::setMotionDetectionDuration(uint8_t dur) {
...
*/
/**************************************************************************/
void Custom_MPU6050::setI2CBypass(bool bypass) {
  Adafruit_BusIO_Register int_pin_config =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_INT_PIN_CONFIG, 1);
...
*/
/**************************************************************************/
bool Custom_MPU6050::enableSleep(bool enable) {
  Adafruit_BusIO_Register pwr_mgmt =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_1, 1);
...
*/
/**************************************************************************/
bool Custom_MPU6050::enableCycle(bool enable) {
  Adafruit_BusIO_Register pwr_mgmt =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_1, 1);
 ...
 */
/**************************************************************************/
mpu6050_cycle_rate_t Custom_MPU6050::getCycleRate(void) {
 ...
 */
/**************************************************************************/
void Custom_MPU6050::setCycleRate(mpu6050_cycle_rate_t rate) {
  Adafruit_BusIO_Register pwr_mgmt_2 =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_2, 1);
 ...
 */
/**************************************************************************/
bool Custom_MPU6050::setGyroStandby(bool xAxisStandby, bool yAxisStandby,
                                      bool zAxisStandby) {
  Adafruit_BusIO_Register pwr_mgmt_2 =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_2, 1);
 ...
 */
/**************************************************************************/
bool Custom_MPU6050::setAccelerometerStandby(bool xAxisStandby,
                                               bool yAxisStandby,
                                               bool zAxisStandby) {
  Adafruit_BusIO_Register pwr_mgmt_2 =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_2, 1);
 ...
 */
/**************************************************************************/
bool Custom_MPU6050::setTemperatureStandby(bool enable) {
  Adafruit_BusIO_Register pwr_mgmt =
      Adafruit_BusIO_Register(i2c_dev, MPU6050_PWR_MGMT_1, 1);
 ...
 */
/**************************************************************************/
void Custom_MPU6050::_read(void) {
  // get raw readings
...
*/
/**************************************************************************/
bool Custom_MPU6050::getEvent(sensors_event_t *accel, sensors_event_t *gyro,
...
void Custom_MPU6050::fillTempEvent(sensors_event_t *temp,
                                     uint32_t timestamp) {
...
void Custom_MPU6050::fillAccelEvent(sensors_event_t *accel,
                                      uint32_t timestamp) {
...
void Custom_MPU6050::fillGyroEvent(sensors_event_t *gyro,
                                     uint32_t timestamp) {
...
*/
Adafruit_Sensor *Custom_MPU6050::getTemperatureSensor(void) {
  return temp_sensor;
}
 ...
 */
Adafruit_Sensor *Custom_MPU6050::getAccelerometerSensor(void) {
  return accel_sensor;
}
 ...
 */
Adafruit_Sensor *Custom_MPU6050::getGyroSensor(void) { return gyro_sensor; }
...
*/
/**************************************************************************/
void Custom_MPU6050_Gyro::getSensor(sensor_t *sensor) {
...
*/
/**************************************************************************/
bool Custom_MPU6050_Gyro::getEvent(sensors_event_t *event) {
  _theMPU6050->_read();
...
*/
/**************************************************************************/
void Custom_MPU6050_Accelerometer::getSensor(sensor_t *sensor) {
  /* Clear the sensor_t object */
  memset(sensor, 0, sizeof(sensor_t));
...
*/
/**************************************************************************/
bool Custom_MPU6050_Accelerometer::getEvent(sensors_event_t *event) {
  _theMPU6050->_read();
...
*/
/**************************************************************************/
void Custom_MPU6050_Temp::getSensor(sensor_t *sensor) {
  /* Clear the sensor_t object */
  memset(sensor, 0, sizeof(sensor_t));
...
*/
/**************************************************************************/
bool Custom_MPU6050_Temp::getEvent(sensors_event_t *event) {
  _theMPU6050->_read();
